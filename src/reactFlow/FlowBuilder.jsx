...
const AddNodeOnEdgeDrop = () => {
  ...
  const handleSubmitGraph = () => {
    ...
    // Update nodes with the current value of node.parameter
    const updatedNodes = nodes.map(node => {
      return {
        ...node,
        data: {
          ...node.data,
          parameter: node.parameter // Add the parameter field to data
        }
      };
    });
  
    object.nodes = updatedNodes;
    object.edges = [...edges];
  
    console.log(object);
  
    axios.post("http://localhost:8080/saveGraph", object)
      .then((response) => {
